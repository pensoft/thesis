...
/*
 * doc className: DBCnMySql Description: MySql Class za vruzka s bazata danni
 * Example: nema
 */
...
class DBCnMySql extends DBCnAbstract {
	...
	function Connect($pSrv, $pDb, $pUsr, $pPass, $pPort) {
		...
		if(MYSQL_CONNECTION_ENCODING){
			mysqli_query($lConnection, 'SET NAMES \'' . MYSQL_CONNECTION_ENCODING . '\'');
		}
		return $lConnection;
	...
	function GetPrototypeSQL($pSqlname, $pActionname, $pSqltype, $pMultilang = 0) {
		...
		$lSqlarr = array(
			...
			GENACT_SP => array(
				...
				"GetData" => "CALL [sqlname]({" . SP_OPID_NAME . "}, " . $lLangParam . "[allvalues]) [filterfields_table]",  // tuka
				                                                                                                            // da
				                                                                                                            // se
				                                                                                                            // opravi
