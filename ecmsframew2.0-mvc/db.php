...
/*
 * doc className: DBCn Description: Class za vruzka s bazata danni Example: nema
 */
...
class DBCn {
	...
	function Open($srv = PGDB_SRV, $db = PGDB_DB, $usr = PGDB_USR, $pass = PGDB_PASS, $port = PGDB_PORT) {
		// function Open($srv = "skylab.etaligent.net", $db = "ropolubo", $usr =
		// "iusrropotamo", $pass = "db^ropotamo?") {
	...
	function CloseRs() {
		if($this->mhRs){
			...
			$this->mLimit = 0;
			if($this->DB->Free_Result($this->mhRs, $this->mhCn))
				$this->mhRs = null;
		}
	}
	...
	function Execute($sqlstr) {
		...
		if(! $this->mhRs){
			$this->mErrMsg = getDbError($this->DB->Last_Error($this->mhCn));
		...
		$t2 = time();
		// ~ trigger_error("Query: ".($t2-$t1)." sec. - $sqlstr ",
		// E_USER_NOTICE);
		$this->MoveFirst();
		return true;
	}
	...
	function Eof() {
		if($this->mPageSize && $this->mCurrentRecord > $this->mPageSize * ($this->mPageNum + 1))
			return true;
	...
	function SetPage($pPageSize, $pPageNum) {
		if($pPageNum == - 1)
			$pPageNum = ceil($this->RecordCount() / $pPageSize) - 1; // kogato
			                                                                          // predadesh
			                                                                          // p
			                                                                          // =
			                                                                          // -1
...
function CustomSQLFunction($fname, $farg, $fieldname, $fieldtype, $dbtype = DEF_DBTYPE, $conArr = null) {
	// ~ trigger_error($fname." ".$farg." ".$fieldname."
	// ".$fieldtype,E_USER_NOTICE);
	if($conArr["dbtype"])
...
function Con($pCustomDBConArr = null) {
	...
	if(! $pCustomDBConArr){
		...
		if(! isset($key)){
			$key = array_push($gCnArr, array(
				"dbobj" => null,
				"dbconarr" => $pCustomDBConArr
			));
			$key --;
		}
...
function GetFilterValueforFilter($value, $type, $name, $customstr, $dbtype = DEF_DBTYPE, $win_encoding = 0) {
	...
	if(! $customstr){
		if($lop == ' like ')
			return "(upper(" . $name . ")" . $lop . "upper(" . SaveSQLValue($value, $type, $name, $dbtype) . "))"; // postgres
			                                                                                                   // bug
			                                                                                                   // v
			                                                                                                   // ilike
			                                                                                                   // i
...
/*
 * function GetCurrentInterval($ptype) - vryshta masiv s start i end na
 * intervala param: ptype - 0 chas, 1 den, 2 sedmcia, 3 mesec, 4 godina
 */
function GetCurrentInterval($ptype) {
	...
	$res = array(
		$larr[0]->format($lformat),
		$larr[1]->format($lformat)
	);
...
function now4hours() {
	...
	// ".($date['hours']+4).":".$date['minutes'].":".$date['seconds'];
	return date('Y-m-d H:i:s', $ltime);
}

function date1() { // Nikola: tva mi trqbva za da filtriram deinosti, koito sa s
                   // nacahlo dnes, no po-golqmo ot now()
	$date = getDate();
	foreach($date as $item => $value){
		if($value < 10)
			$date[$item] = "0" . $value;
...
// Denitsa - da polu4avam razni stringove za def.xml (v spravkite - proekta =
// teku6tata godina i 1(20091)
