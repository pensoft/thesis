...
<tr>
	<td>{guid}</td>
	<td><a href="/resources/video/edit.php?tAction=showedit&guid={guid}">{title}</a></td>
	<td>{uname}</td>
	<td>{author}</td>
	...
	<td align="right" nowrap="true">
		...
		' : '
			<a href="/resources/video/edit.php?tAction=showedit&guid={guid}"><img src="/img/edit.gif" alt="Редактирай" title="Редактирай" border="0" /></a>
			<a href="/resources/video/edit.php?tAction=delete&guid={guid}" onclick="javascript: if (confirm(\'Сигурни ли сте, че искате да изтриете това видео?\')) { return true; } else { return false;}"><img src="/img/trash2.gif" alt="Изтрий" title="Изтрий" border="0" /></a>
		') . ' 
	</td>
</tr>
';
			...
			<table cellspacing="0" cellpadding="5" border="0" class="gridtable">
				<tr>
					<th class="gridtools" colspan="8">
						<a href="' . $lAddStoryUrl . '">' . getstr('admin.videos.addStory') . '</a>
						' . getstr('admin.videos.antetka') . '
					</th>
				</tr>
';
...
if (!$l->DisplayList($page)) {
	echo $lTableHeader . '<tr><td colspan="8"><p align="center"><b>' . getstr('admin.videos.noData') . '</b></p></td></tr>' . $lTableFooter;
}
