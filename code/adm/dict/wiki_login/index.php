...
$gKforFlds = array(	
	...
	'name' => array(
		'VType' => 'string',
		'CType' => 'text',
		'DisplayName' => getstr('admin.wiki_login.name'),
		'AddTags' => array(
			'class' => 'coolinp',
		),
		'AllowNulls' => true,
	...
	<div class="r">
		...
		<div class="tr">
			<table cellspacing="0" cellpadding="5" border="0" class="formtable">
				...
				<tr>
					<th colspan="2">' . getstr('admin.wiki_login.filter') . '</th>
				</tr>
				<tr>
					<td>{*name}<br/>{name}</td>					
				</tr>
...
if( $gKfor->lCurAction == 'show' && !$gKfor->lErrorCount){
	if( trim($gKfor->lFieldArr['name']['CurValue'])){		
		$lWhereArr[] = ' n.username ILIKE  \'%' . q(trim($gKfor->lFieldArr['name']['CurValue'])) . '%\'';
	}
...
<tr>
	...
	<td align="right" nowrap="true">
			...
			<a href="javascript:if (confirm(\'' . getstr('admin.wiki_login.ConfirmDel') . '\')) { window.location = \'/dict/wiki_login/edit.php?id={id}&tAction=delete\';} else {}">
...
$lListAntets = array(
	1 => array('caption' => getstr('admin.wiki_login.colID'), 'deforder' => 'desc', 'def'), 
	2 => array('caption' => getstr('admin.wiki_login.colUserName'), 'deforder' => 'asc'), 	
			...
			<table cellspacing="0" cellpadding="5" border="0" class="gridtable">
				<tr>
					<th class="gridtools" colspan="10">
						<a href="./edit.php">' . getstr('admin.wiki_login.addItem') . '</a>
						' . getstr('admin.wiki_login.antetka') . '
					</th>
				</tr>
';
...
$lListSql = 'SELECT n.id, n.username as name
FROM wiki_login n
' . $gListSqlWhere;
//~ echo $lListSql;
...
if (!$lList->DisplayList($gListPage)) {
	echo $lTableHeader . '<tr><td colspan="4"><p align="center"><b>' . getstr('admin.wiki_login.noData') . '</b></p></td></tr>' . $lTableFooter;
}
