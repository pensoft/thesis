...
$fld = array(
	'level' => array (
		...
		'CType' => 'select' ,
		'SrcValues' =>	'
		SELECT null as id, \'' . getstr('admin.secsites.basicMenus') . ' + *\' as name UNION
		...
		'DisplayName' => getstr('admin.secsites.level'),
	),
			...
			<table cellspacing="0" cellpadding="5" border="0" class="gridtable" id="ordtable">
				<tr>
					<th class="gridtools" colspan="7">
						<a href="./edit.php">' . getstr('admin.secsites.addNewSite') . '</a>
						' . getstr('admin.secsites.antentka') . '
					</th>
				</tr>
				<tr>
					<th myhiddenname="ordstr">&nbsp;</th>
					<th>' . getstr('admin.secsites.colIDLabel') . '</th>
					<th>' . getstr('admin.secsites.colSiteLabel') . '</th>
					<th>' . getstr('admin.secsites.colSubdirLabel') . '</th>
					<th>' . getstr('admin.secsites.colSubLevelsLabel') . '</th>
					<th>' . getstr('admin.secsites.colTypeLabel') . '</th>
					<th>&nbsp;</th>
				</tr>
';
...
if (!$gCn->mRecordCount) {
	echo '<tr><td colspan="8"><p align="center"><b>' . getstr('admin.secsites.noData') . '</b></p></td></tr>';
} else {
	while( ! $gCn->EoF() ) {
		
		$subrubr = getstr('admin.secsites.noSubLevelsLabel');
		if ($gCn->mRs['sublevel']) $subrubr = '<a href="?level=' . $gCn->mRs['id'] . '&tAction=show">' . getstr('admin.secsites.seeButton') . '</a>';
		...
		echo '
			<tr bgcolor="#' . ( $lCounter % 2 == 1 ? 'ffffff' : '96b9df' ) . '" id="r' . $gCn->mRs['id'] . '">
				...
				<td align="right"><a href="./edit.php?id=' . $gCn->mRs['id'] . '&tAction=show"><img src="/img/edit.gif" border="0" alt="' . getstr('admin.editLabel') . '" title="' . getstr('admin.editLabel') . '" /></a></td>
			</tr>';
		$gCn->MoveNext();
		$lCounter++;
	}
...
function getLinkType($pType){
	if( (int) $pType == 1)
		return getstr('admin.secsites.linkType');
	return getstr('admin.secsites.delimiterType');
}
