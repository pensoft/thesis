...
ADD MORE QUESTIONS IN TYPE ret_revieweranswers AND IN pjs.spGetReviewerAnswers 

**************************************/


--DROP ret_revieweranswers CASCADE;
...
BEGIN
	...
	FOR r IN SELECT 1 as question, drr.question1, COUNT(drr.question1) as count FROM pjs.document_users du
		...
		LEFT JOIN pjs.document_review_round_users_form drr ON (drr.document_review_round_user_id = dr.id)
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION pjs.spGetReviewerAnswers(integer) OWNER TO postgres;
GRANT EXECUTE ON FUNCTION pjs.spGetReviewerAnswers(integer) TO postgres;
GRANT EXECUTE ON FUNCTION pjs.spGetReviewerAnswers(integer) TO iusrpmt;
GRANT EXECUTE ON FUNCTION pjs.spGetReviewerAnswers(integer) TO pensoft;
