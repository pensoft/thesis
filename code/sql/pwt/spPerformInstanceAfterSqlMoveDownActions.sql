DROP TYPE ret_spPerformInstanceAfterSqlMoveDownActions CASCADE;
CREATE TYPE ret_spPerformInstanceAfterSqlMoveDownActions AS (
	result int
);

CREATE OR REPLACE FUNCTION spPerformInstanceAfterSqlMoveDownActions(
	pUid int,
	pInstanceId bigint,
	pMode int DEFAULT 1
)
  RETURNS ret_spPerformInstanceAfterSqlMoveDownActions AS
$BODY$
	DECLARE
		lRes ret_spPerformInstanceAfterSqlMoveDownActions;	
...
GRANT EXECUTE ON FUNCTION spPerformInstanceAfterSqlMoveDownActions(	
	pUid int,
	pInstanceId bigint,
	pMode int
) TO iusrpmt;
