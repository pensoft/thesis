DROP TYPE pjs.ret_spImportVersionComments CASCADE;
CREATE TYPE pjs.ret_spImportVersionComments AS (
	result int
);

CREATE OR REPLACE FUNCTION pjs.spImportVersionComments(
	pVersionId bigint,
	pCommentsXml xml
)
  RETURNS pjs.ret_spImportVersionComments AS
$BODY$
DECLARE
	lRes pjs.ret_spImportVersionComments;
	lRecord record;
	lComments xml[];
	lTemp xml[];
...
BEGIN
	...
	CREATE TEMP TABLE pjs_comments_import (
		LIKE pjs.msg,
		new_id int
	);
	...
	SELECT INTO lDocumentId 
		document_id
	FROM pjs.document_versions WHERE id = pVersionId;
	
	IF lDocumentId IS NULL THEN
		RETURN lRes;
	END IF;
	
	DELETE FROM pjs.msg WHERE version_id = pVersionId;
	...
	LOOP
		...
		RAISE NOTICE 'id %, usr_id %, ord %, msg %, flags %, rootid %, mdate %, start_object_instances_id %, start_object_field_id %, start_offset %,
			...
			lEndInstanceId, lEndFieldId, lEndOffset, lDocumentId, pVersionId;
		*/	
		
		INSERT INTO pjs_comments_import(id, usr_id, ord, msg, flags, rootid, mdate, start_object_instances_id, start_object_field_id, start_offset,
			end_object_instances_id, end_object_field_id, end_offset, document_id, version_id,
			subject, lastmoddate, original_id, is_resolved, resolve_uid, resolve_date, is_disclosed, undisclosed_usr_id)
		VALUES(lPreviousId, lUsrId, lOrd, coalesce(lMsg, ''), lFlags, lRootId, lMdate, lStartInstanceId, lStartFieldId, lStartOffset,
			lEndInstanceId, lEndFieldId, lEndOffset, lDocumentId, pVersionId,
			'', now(), lPreviousId, lIsResolved::boolean, lResolveUid, lResolveTime, lIsDisclosed::boolean, lUndisclosedUid);
	END LOOP;
	
	FOR lRecord IN 
		SELECT * FROM pjs_comments_import
	LOOP
		lMsgID := nextval('pjs.msg_id_seq');
		
		INSERT INTO pjs.msg(id, usr_id, ord, msg, flags, rootid, mdate, 
			start_object_instances_id, start_object_field_id, start_offset, 
			end_object_instances_id, end_object_field_id, end_offset, document_id, version_id,
			subject, original_id, is_resolved, resolve_uid, resolve_date, is_disclosed, undisclosed_usr_id)
		VALUES(lMsgID, lRecord.usr_id, lRecord.ord, lRecord.msg, lRecord.flags, lMsgID, lRecord.mdate, 
			CASE WHEN coalesce(lRecord.start_object_instances_id, 0) > 0 THEN lRecord.start_object_instances_id ELSE NULL END, lRecord.start_object_field_id, lRecord.start_offset,
			CASE WHEN coalesce(lRecord.end_object_instances_id, 0) > 0 THEN lRecord.end_object_instances_id ELSE NULL END, lRecord.end_object_field_id, lRecord.end_offset, lDocumentId, pVersionId, 
			lRecord.subject, lMsgID, lRecord.is_resolved, CASE WHEN coalesce(lRecord.resolve_uid, 0) > 0 THEN lRecord.resolve_uid ELSE NULL END, lRecord.resolve_date, lRecord.is_disclosed, CASE WHEN coalesce(lRecord.undisclosed_usr_id, 0) > 0 THEN lRecord.undisclosed_usr_id ELSE NULL END);
					
		
		UPDATE pjs_comments_import SET
			new_id = lMsgID
		WHERE id = lRecord.id;
	END LOOP;
	...
	UPDATE pjs.msg m SET
		rootid = i1.new_id
	FROM pjs_comments_import i 
	JOIN pjs_comments_import i1 ON i1.id = i.rootid
	WHERE i.new_id = m.id;
...
GRANT EXECUTE ON FUNCTION pjs.spImportVersionComments(
	pVersionId bigint,
	pCommentsXml xml
) TO iusrpmt;
