...
class cdocument_figures extends crs_array {
	function GetData() {
...
// 		echo 'After ' .  date("Y/m/d H:i:s"). substr((string)microtime(), 1, 6) . "\n";
		if ($this->m_state >= 1) {
			$lPreviousRow = null;
// 			error_reporting(-1);
			$lHtml = getDocumentFiguresPreview($this->m_pubdata['document_id']);
			...
			if($lDom->loadHTML($lHtml)){
				...
				foreach($this->m_resultArr as &$lCurrentRow) {
					...
					$lPreviewNode = $lXPath->query('//div[@id="Figure-Preview-Wrapper' . $lItemId . '"]');
