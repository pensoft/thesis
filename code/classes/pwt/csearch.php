...
class csearch extends ct2search {
	function __construct($pFieldTempl) {
		...
		global $user;
		if(isset($pFieldTempl['sqlstr'])) {
			$this->m_pubdata['addwhere'] = ' AND du.usr_id = ' . q($user->id) . '
												GROUP BY d.id, d.name, d.lastmoddate';
			$this->m_pubdata['sqlstr'] = $pFieldTempl['sqlstr'];
			$this->m_pubdata['orderby'] = ' ORDER BY d.lastmoddate DESC';
		} else {
