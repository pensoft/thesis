...
class cdocument_supfiles extends crs_array {
	function GetData() {
...
// 			error_reporting(-1);
			$lHtml = getDocumentSupFilesPreview($this->m_pubdata['document_id']);
			...
			if($lDom->loadHTML($lHtml)){
				...
				foreach($this->m_resultArr as &$lCurrentRow) {
					...
					$lPreviewNode = $lXPath->query('//div[@id="Sup-File-Preview-Wrapper' . $lItemId . '"]');
