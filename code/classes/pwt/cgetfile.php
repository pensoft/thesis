...
class cgetfile extends cbase {
	...
	function GetData($pCode = 'null') {
		...
		$cn = Con();
		$lSqlStr = 'SELECT * from pwt.media WHERE id = ' . (int)$this->file_id ;
		$cn->Execute($lSqlStr);
		$cn->MoveFirst();
			
		if (!$cn->Eof()) {
			
			$this->filename = $cn->mRs['original_name'];
			
			if ($cn->mRs['title']) {
				$this->filetitle = $cn->mRs['title'];
			} else {
	...
	function sendheaders($pType) {
		if ($pType == 'def') {
			...
			readfile(PATH_PWT_DL . $this->filename);
