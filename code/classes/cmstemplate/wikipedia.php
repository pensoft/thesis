...
class http {
    ...
    function post ($url,$data, $contenttype = 'multipart/form-data') {
        //~ echo 'POST: '.$url."\n";
        $time = microtime(1);
...
class wikipedia {
     ...
     **/
    function query ($query,$post=null, $contenttype = 'multipart/form-data') {
        if ($post==null)
            $ret = $this->http->get($this->url.$query);
        else 
            $ret = $this->http->post($this->url.$query,$post, $contenttype);
    ...
    **/
    ...
    function login ($user,$pass) {
        ...
        if ($ret['login']['result'] != 'Success') {
            throw new Exception('Wiki login error');
        } else {
            return $ret;
        }
    }
	...
	function getimporttoken () {
        $x = $this->query('?action=query&prop=info&intoken=import&titles=Main%20Page&format=php');
		//~ var_dump($x);
        foreach ($x['query']['pages'] as $ret) {
            return $ret['importtoken'];
        }
    }
    ...
    /**
    * Import a XML file built with export
    * @param $pXmlFile Path to XML file which you want to upload.
    * @return api result
    **/
    function import($pXmlFile) {
        if ($this->token==null) {
            $this->token = $this->getimporttoken();
        }
        $params = array(
			'token' => $this->token,
			'xml' => "@".$pXmlFile,
        );
        return $this->query('?action=import&format=php', $params, 'multipart/form-data');
    }
    ...
    **/
     ...
     **/
     function upload ($page,$file,$desc='') {
      ...
      /**
     * Uploads an image from url address not from local file.
     * @param $page The destination file name.
     * @param $pPicUrl The local file path.
     * @param $desc The upload discrption (defaults to '').
     **/
     function uploadPicFromUrl ($page,$pPicUrl,$desc='') {
        if ($this->token == null) {
                $this->token = $this->getedittoken();
        }
        $params = array(
