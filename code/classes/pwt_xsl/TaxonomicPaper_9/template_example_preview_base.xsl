	...
	<xsl:template match="/document">
		<xsl:variable name="lContent">
			<div class="P-Article-Preview">
				...
				<div>
					<xsl:attribute name="instance_id"><xsl:value-of select="/document/objects/*[@object_id='14']/*[@object_id='9']/@instance_id"/></xsl:attribute>
					<div>
...
<!-- 				<xsl:apply-templates select="/document/tables/table" mode="tables"/> -->
			...
			<xsl:when test="$gGenerateFullHtml &gt; 0">
				<html>
					<head>
						...
						<title>
							<xsl:value-of select="php:function('strip_tags', string(/document/objects/*[@object_id='14']/*[@object_id='9']/fields/*[@id='3']))"/>
						</title>
					</head>
					<body>
						<xsl:comment>
							<xsl:value-of select="$pEditableHeaderReplacementText"/>
						</xsl:comment>
	...
	<xsl:template match="*[@object_id='18']" mode="bodySections">
		<xsl:if test="./fields/*[@id='22']/value != '' or ./subsection/@object_id != ''">
			<div class="P-Article-Preview-Block">
				...
				<h1 id="materials_and_methods">Materials and methods</h1>
				<xsl:if test="./fields/*[@id='22']/value  != ''">
					<div class="Section" field_id="22">
						<xsl:call-template name="markContentEditableField">
							<xsl:with-param name="pObjectId" select="18"/>
							<xsl:with-param name="pFieldId" select="22"/>
						</xsl:call-template>
	...
	<xsl:template match="*[@object_id='17']" mode="bodySections">
		<xsl:if test="./fields/*[@id='21']/value != '' or ./subsection/@object_id != ''">
			<div class="P-Article-Preview-Block">
				<xsl:attribute name="instance_id"><xsl:value-of select="./@instance_id"/></xsl:attribute>
				<h1 id="data_resources">Data resources</h1>
		...
		<xsl:if test="./fields/*[@id='224']/value != '' or ./subsection/@object_id != ''">
			<div class="P-Article-Preview-Block">
				...
				<xsl:if test="./fields/*[@id='224']/value != ''">
					<div class="Section" field_id="224">
						<xsl:call-template name="markContentEditableField">
							<xsl:with-param name="pObjectId" select="./@object_id"/>
							<xsl:with-param name="pFieldId" select="224"/>
						</xsl:call-template>
	...
	<xsl:template match="*" mode="singleIdentificationKey">
		<table cellspacing="0" cellpadding="0" border="0" width="100%" style="border-collapse:collapse;" identification_key_table="1">
			<xsl:attribute name="instance_id"><xsl:value-of select="./@instance_id"/></xsl:attribute>
			<tbody>
				<tr>
	...
	<xsl:template match="*" mode="identificationKeyCouplet" name="identificationKeyCouplet">
		...
		<tr>
			<xsl:attribute name="instance_id"><xsl:value-of select="./@instance_id"/></xsl:attribute>
			<td class="P-Article-Preview-Table-Row" align="center">
				<xsl:value-of select="$pNum"/>
			</td>
			<td class="P-Article-Preview-Table-Row">
				<xsl:call-template name="markContentEditableField">
					<xsl:with-param name="pObjectId" select="$pNode/@object_id"/>
	...
	<xsl:template match="*" mode="singleIdentificationKeyCouplet" name="singleIdentificationKeyCouplet">
		<xsl:variable name="lInstanceId" select="./@instance_id"/>
		<xsl:variable name="lParentInstanceId" select="./ancestor::*[@object_id='23']/@instance_id"/>
	...
	<xsl:template match="*[@object_id='203']" mode="bodySections">
		<xsl:if test="count(./*[@object_id='204']) &gt; 0">
			...
			<div class="P-Article-Preview-Block" id="checklists">
				<xsl:attribute name="instance_id"><xsl:value-of select="./@instance_id"/></xsl:attribute>
	...
	<xsl:template match="*[@object_id='54']" mode="bodySections">
		<xsl:if test="(count(./*[@object_id='41']) &gt; 0) or (./fields/*[@id='40']/value != '')">
			<div class="P-Article-Preview-Block">
				<xsl:attribute name="instance_id"><xsl:value-of select="./@instance_id"/></xsl:attribute>
				<h1 id="taxon_treatments">Taxon treatment<xsl:if test="(count(./*[@object_id='41']) &gt; 1)">
						<xsl:text>s</xsl:text>
					</xsl:if>
				</h1>
	...
	<xsl:template match="*" mode="taxonTreatment">
		...
		<div class="taxonTreatment">
			...
			<xsl:apply-templates select=".//*[@object_id='214']/*[@object_id=48]" mode="taxonTreatmentSections"/>
			<!-- missing -->
			...
			<xsl:apply-templates select=".//*[@object_id='214']/*[@object_id=80]" mode="taxonTreatmentSections"/>
			<!-- missing -->
			...
			<xsl:apply-templates select=".//*[@object_id='193']/*[@object_id=75]" mode="taxonTreatmentSections"/>
			<!-- missing -->
	...
	<xsl:template match="*" mode="TTExternalLinks">
		...
		<xsl:variable name="lTreatmentURLPrefix">
			<xsl:choose>
				...
				<xsl:when test="$lTreatmentURLType='3'">
					<xsl:text>http://www.ncbi.nlm.nih.gov/nuccore/</xsl:text>
				</xsl:when>
				<xsl:when test="$lTreatmentURLType='4'">
					<xsl:text/>
				</xsl:when>
				<xsl:when test="$lTreatmentURLType='5'">
					<xsl:text>http://ipni.org/</xsl:text>
				</xsl:when>
				<xsl:when test="$lTreatmentURLType='6'">
					<xsl:text>http://www.mycobank.org/MB/</xsl:text>
				</xsl:when>
				<xsl:when test="$lTreatmentURLType='7'">
					<xsl:text>http://www.indexfungorum.org/names/NamesRecord.asp?RecordID=</xsl:text>
				</xsl:when>
				<xsl:when test="$lTreatmentURLType='8'">
					<xsl:text>http://www.barcodinglife.org/index.php/Public_RecordView?processid=</xsl:text>
				</xsl:when>
				<xsl:otherwise>
					<xsl:text/>
				</xsl:otherwise>
			...
			<xsl:if test="count(./*[@object_id='219']/*[@object_id='218']) &gt; 0">
				<div class="treatmentSection" style="color:#404040">
					<h3 class="h-treatment-section">
						<i>Synonymys of the type species</i>
					</h3>
					<xsl:for-each select="./*[@object_id='219']/*[@object_id='218']">
						<div class="typeSpeciesIndent">
							<xsl:apply-templates select="." mode="Synonymy"/>
						</div>
					</xsl:for-each>
				</div>
			</xsl:if>
