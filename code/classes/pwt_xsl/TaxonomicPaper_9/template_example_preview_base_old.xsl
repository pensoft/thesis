...
<!-- 		</xsl:element> -->
							...
							<div class="P-Article-Preview-Block-Content">
								<div class="MaterialType">
									<i>
										...
										<xsl:if test="(count($lTreatmentNode//*[@object_id='37'][./*/fields/*[@id='209']/value[@value_id=$lMaterialTypeId]]) &gt; 1)"><xsl:text>s</xsl:text></xsl:if><xsl:text>: </xsl:text>
		...
		<div class="P-Article-Preview-Block">
			...
			<xsl:if test="count(.//*[@object_id='195']) &gt; 0 or 
				...
				<div class="P-Article-Preview-Block-Content">
					...
					<xsl:apply-templates select=".//*[@object_id='195' or @object_id='186' or @object_id = '215' or @object_id = 217]" mode="taxonTypeSpecies"/>
				</div>
			</xsl:if>
			...
			<xsl:if test="count(.//*[@object_id='37']) &gt; 0">
				<div class="P-Article-Preview-Block-Content">
					<h3 class="h-treatment-section">Materials</h3>
	...
	<xsl:template match="*[@object_id='181']" mode="taxonTreatmentName">
		...
		<span>
			<xsl:attribute name="instance_id"><xsl:value-of select="./@instance_id" /></xsl:attribute>
			<span><xsl:attribute name="field_id">50</xsl:attribute><xsl:apply-templates select="./fields/*[@id='50']" mode="formatting"/></span></span>
	</xsl:template>
	...
	<xsl:template match="*" mode="taxonTypeSpecies">
		...
		<span class="P-Inline">
			...
			<xsl:if test="./fields/*[@id='440']/value/@value_id = 1"> <!-- new species described in this paper -->
				<xsl:choose>
	...
	<xsl:template match="*" mode="taxonSynonymsSections">
		<xsl:variable name="lSecTitle"></xsl:variable>
		<xsl:if test="./fields/*[@id='460']/value != ''">
			<div class="P-Article-Preview-Block-Content">
				...
				<div class="P-Article-Preview-Block-Subsection-Title">
					...
					<h3 class="h-treatment-section">Synonyms</h3>
				</div>
				<div>
	...
	<xsl:template match="*[@object_id='202']" mode="articleBack">
		<xsl:if test="./fields/*[@id='464']/value != ''">
			<div class="P-Article-Preview-Block">
				<xsl:attribute name="instance_id"><xsl:value-of select="./@instance_id" /></xsl:attribute>
				<h1 id="authorContributions">Author contributions</h1>
				<div class="P-Article-Preview-Block-Content">
					<xsl:call-template name="markContentEditableField">
						<xsl:with-param name="pObjectId" select="./@object_id"></xsl:with-param>
						<xsl:with-param name="pFieldId">464</xsl:with-param>
					</xsl:call-template>
					<xsl:attribute name="field_id">464</xsl:attribute>
	...
	<xsl:template match="*[@object_id='57']" mode="articleBack">
		<xsl:if test="./fields/*[@id='223']/value != ''">
			<div class="P-Article-Preview-Block">
				<xsl:attribute name="instance_id"><xsl:value-of select="./@instance_id" /></xsl:attribute>
				<h1 id="acknowledgements">Аcknowledgements</h1>
				<div class="P-Article-Preview-Block-Content">
					<xsl:call-template name="markContentEditableField">
						<xsl:with-param name="pObjectId" select="./@object_id"></xsl:with-param>
