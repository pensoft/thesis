...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:tp="http://www.plazi.org/taxpub"  xmlns:php="http://php.net/xsl" exclude-result-prefixes="php tp xlink xsl">
			...
			<div class="P-Article-Preview-Block-Content">
				<xsl:for-each select=".">
					...
					<div class="P-Article-Preview-Block-Content">	
						...
						<xsl:apply-templates select=".//*[@object_id='84']" mode="treatmentMaterialCust"></xsl:apply-templates>
					</div>						
				</xsl:for-each>
			</div>
			</xsl:if>
	...
	<xsl:template match="*" mode="treatmentMaterialCust">
		...
		<div class="P-Article-Preview-Block-Content P-Inline">
			<span>
				<xsl:attribute name="instance_id"><xsl:value-of select="./@instance_id" /></xsl:attribute>
				<xsl:for-each select=".//fields/*[value != '' ][@id != '209']">
	...
	<xsl:template match="*" mode="synonymyLittle">
			<!-- Taxon name -->
			
			<span field_id="48" ><xsl:value-of select="./*[@object_id = 180]/fields/*[@id = '48']/value"  /></span>
			<xsl:if test="./*[@object_id = 180]/fields/*[@id = '417']/value != ''">
			<span field_id="417"> (<xsl:value-of select="./*[@object_id = 180]/fields/*[@id = '417']/value" />)</span>
			</xsl:if>
			<span field_id="49" >&#160;<xsl:value-of select="./*[@object_id = 180]/fields/*[@id = '49']/value"  /></span>
			<span field_id="50" >, <xsl:value-of select="./*[@object_id = 180]/fields/*[@id = '50']/value"  /></span>
			<!-- Citations-->
			<!--<xsl:for-each select="/*[@object_id = 187/*[@object_id = 201]/">
				<xsl:apply-templates select="." mode="synonymyCitation" />
			</xsl:for-each>-->
	</xsl:template>
