...
if(isset($_FILES['uploadfile']) && (int)$gDocumentId && $gInstanceId) {
	...
	$lData = CreateNewTaxonomicCoverageTaxaFromSpreadSheet('uploadfile', PATH_PWT_UPLOADED_FILES, $gDocumentId);
	$lResult['file_path'] = PATH_PWT_UPLOADED_FILES . $lData['file_name'];

	if($lResult['file_path'] && !$lResult['err'] ) {
		$lImportMaterial = new cimport_data_paper_taxonomic_coverage(
			array(
