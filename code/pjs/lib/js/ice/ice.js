...
//			var p = this.env.document.createElement(this.blockEl);
//			p = body.appendChild(p);
//			ice.dom.each(ice.dom.contents(this.element), function(i, node) {
////				if(ice.dom.isBlockElement(node) || ice.dom.isTextNode(node))
...
//		this.element.innerHTML = body.innerHTML;
	...
	/**
	 * Returns the html content of the root element
	 */
	getElementContents: function(){
		//Autoclose all br tags
		var lFakeThis = this;
...
//			console.log(1);
		...
		ice.dom.each(ice.dom.find(lChangesSelector, '.' + gActiveCommentTextClass), function(i, node){
			$(node).attr('title', '');
			$(node).attr(gChangeUserNamesAttrName, '');
		})


		ice.dom.each(ice.dom.find(lElementCopy, 'br'), function(i, node){
			if(!node.hasChildNodes()){
//				var lNewNode = ice.dom.create('<br/>');
//				node.parentNode.replaceChild(lNewNode, node)
				$(node).html(' ');
			}
		})
		var lResult = $(lElementCopy).html();
		var lBrRegExpr = new RegExp("<br\\s*>(?!<\/br>)", "g")
		var lImgRegExpr = new RegExp("<img([^>]*)>", "g")
		lResult = lResult.replace(lBrRegExpr, '<br/>');
		function ImgReplace(pMatchedStr, pGroup1) {
		    if(pGroup1[pGroup1.length - 1] == '/'){
		    	return pMatchedStr;
		    }
			return pMatchedStr + '</img>';
		}
		lResult = lResult.replace(lImgRegExpr, ImgReplace);
		return lResult;
//		return $(this.element).find(this.blockEl).first().html();
	},
	...
	getCleanContent: function(body, callback, prepare) {
		...
		ice.dom.each(changes, function(i, node) {
			if(this.mode = gAuthorRoleViewMode){
				ice.dom.replaceWith(this, ice.dom.contents(this));
			}else{
				lEditor.acceptChangeInSEMode(node, true);
			}
		});
		...
		if(this.mode = gAuthorRoleViewMode){
			...
			ice.dom.each(deletes, function(i, node) {
				lEditor.acceptChangeInSEMode(node, false);
			});
		}
	...
	acceptChangeInSEMode: function(pNode, pChangeIsInsert){
		var lEditor = this;
		var lNodeName = gAcceptedDeleteChangeNodeName;
		if(pChangeIsInsert){
			lNodeName = gAcceptedInsertChangeNodeName;
		}
		var lNewElement = document.createElement(lNodeName);
		lNewElement.setAttribute(lEditor.changeIdAttribute, $(pNode).attr(lEditor.changeIdAttribute));
		lNewElement.setAttribute(lEditor.userNameAttribute, $(pNode).attr(lEditor.userNameAttribute));
		lNewElement.setAttribute(lEditor.userIdAttribute, $(pNode).attr(lEditor.userIdAttribute));
		lNewElement.setAttribute('alt-title', $(pNode).attr('title'));
		$(lNewElement).html(ice.dom.contents(pNode));
		ice.dom.replaceWith(pNode, $(lNewElement));
	},
	...
	acceptRejectChange: function(node, isAccept) {
		...
		if(!isAccept || this.mode == gAuthorRoleViewMode){
			...
			//Accept change in SE view
			var lEditor = this;
			if(ice.dom.is(trackNode, replaceSel) || dom.is(trackNode, removeSel)){
				var lChangeIsInsert = true;
				if(ice.dom.is(trackNode, removeSel)){
					lChangeIsInsert = false;
				}
				dom.each(changes, function(i, node) {
					lEditor.acceptChangeInSEMode(node, lChangeIsInsert);
				});
	...
	setUserStyle: function(userid, styleIndex) {
		var style = this.stylePrefix + '' + userid;
