...
function checkSessionLangs() {
	if (is_array($_SESSION['langs']) && count($_SESSION['langs']))  return;
...
function getlang($code=false) {
	static $curlangcode;
	static $curlangid;
	if ($curlangcode) return ($code ? $curlangcode : $curlangid);
	//$curlangcode;
	checkSessionLangs();
	if ($_REQUEST['lang'] && (-1<($lid=getlangidbycode($_REQUEST['lang'])))) {
 ...
 * @param  int $pLangId - ID of a specific language
 * @param  string $pPropName - Property name of the specified language
 * @return mixed
 */
...
function negotiate_language() {
	/*if ($_SERVER[HTTP_ACCEPT_LANGUAGE]) {
		...
		foreach($langarr as $k => $v) {
			$lng=split("[-;]",$v,1);
			if (-1<=($lid=getlangidbycode($lng[0]))) {
				$_SESSION['CURLANG']=$_SESSION['langs'][$lid];
				return ;
			}
		}
