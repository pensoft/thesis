...
className: DBCnPostgres

Description: Postgres Class za vruzka s bazata danni
...
class DBCnPostgres extends DBCnAbstract{
	...
	/*function Fetch_Array($pmhRs,$pcount) {
		return pg_fetch_array($pmhRs);
	}*/
...
class PostgresHelper extends DBCnHelperAbstract{
	...
	function SaveSQLValue($value,$type,$name,$pCon=null, $win_encoding=0) { 
		...
		if($type=='time'){
			if(!preg_match("/^\'[0-1][0-9]:[0-5][0-9](:[0-5][0-9])?\'$/",$value)){
				if(!preg_match("/^\'2[0-3]:[0-5][0-9](:[0-5][0-9])?\'$/",$value))$errmsg=$name.' - Not a valid time value: '.$value;
			}
		}
	...
	function CustomSQLFunction($fname,$farg,$fieldname,$fieldtype) {
		if (($fname=="groupby") && (($fieldtype=="timestamp") || ($fieldtype=="date") || ($fieldtype=="datetime")))  {
			...
			//~ trigger_error($farg." - date_trunc('".$period."', ".$fieldname.")",E_USER_NOTICE);
	...
	function GetPrototypeSQL($sqlname,$actionname,$sqltype, $multilang=0) {
		...
		$sqlarr=array (
			...
			GENACT_SP =>array(
				...
				"GetData"=>"select * from [sqlname]({".SP_OPID_NAME."}, ".($multilang ?"{__lang}, ":"")."[allvalues]) [filterfields_table]", //tuka da se opravi
